...
namespace GameDonkeyLib
{
	public class CreateAttackAction : BaseAction
	{
		...
		/// </summary>
		private List<BaseAction> m_listSuccessActions;
		...
		protected virtual bool ReadActionAttribute(XmlNode childNode, IGameDonkey engine, SingleStateContainer stateContainer, ContentManager content)
		{
			...
			else if ((strName == "hitSound") && !string.IsNullOrEmpty(strValue))
			{
				...
				HitSound = engine.LoadSound(HitSoundName);
			}
			else if (strName == "successActions")
			{
				//Read in all the success actions
				if (!BaseAction.ReadXmlListActions(Owner, ref m_listSuccessActions, childNode, engine, stateContainer, content))
				{
					Debug.Assert(false);
					return false;
				}
