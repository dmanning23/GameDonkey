...
namespace GameDonkey.Tests
{
    ...
    public class ConstantDeccelerationTests
    {
        ...
        public void Persist()
        {
            ...
            var container = new StateContainerModel(new Filename("ConstantDeccelerationTests.xml"));
            ...
            //read it back in
            var container2 = new StateContainerModel(new Filename("ConstantDeccelerationTests.xml"));
            container2.ReadXmlFile();

            //get the action
            container2.StatesActions.Count.ShouldBe(1);
            container2.StatesActions[0].ActionModels.Count.ShouldBe(1);
            var model2 = container2.StatesActions[0].ActionModels[0] as ConstantDeccelerationActionModel;
        ...
        [Test]
        public void Execute_SetsDeccelActionOnOwner()
        {
            var owner = new TestObject();
            var action = new ConstantDeccelerationAction(owner);

            action.Execute(0f);

            owner.DeccelAction.ShouldBeSameAs(action);
        }
    }
}
